...
static uint32_t pixel_buffer_1[256 * 448], pixel_buffer_2[256 * 448];
        ...
        switch (event.type) {
            ...
            case SDL_MOUSEMOTION: {
                if (GB_has_accelerometer(gb) && configuration.allow_mouse_controls) {
                    ...
                    y = SDL_max(SDL_min(y, GB_PIXELS_V), 0);
...
int main(int argc, char **argv)
{
    ...
    window = SDL_CreateWindow("SameBoy v" GB_VERSION, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                              160 * configuration.default_scale, GB_PIXELS_V * configuration.default_scale, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
    if (window == NULL) {
        fputs(SDL_GetError(), stderr);
        exit(1);
    }
    SDL_SetWindowMinimumSize(window, 160, GB_PIXELS_V);
    ...
    if (gl_context == NULL) {
        ...
        texture = SDL_CreateTexture(renderer, SDL_GetWindowPixelFormat(window), SDL_TEXTUREACCESS_STREAMING, 160, GB_PIXELS_V * 2);
